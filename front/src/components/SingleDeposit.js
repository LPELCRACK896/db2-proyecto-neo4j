...
const SingleDeposit = () => {
  ...
  const getClient = async () => {
    try {
      const res = await axios.get(`http://localhost:5000/api/v1/deposits/${id}`);
      const data = res.data;
  ...
  return (
    <div className="single-client-container">
      ...
      <div className="client-buttons">
        <Link to={`/deposits/${id}/edit`} className="edit-button">Editar</Link>
      ...
      <div className="client-info">
        ...
        <p><strong>Location X:</strong> {client.location.x}</p>
        <p><strong>Location Y:</strong> {client.location.y}</p>
